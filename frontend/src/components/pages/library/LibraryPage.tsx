...
const LibraryPage: React.FC = () => {  
  ...
  const fetchBooks = async (page: number) => {
    ...
    setLoading(true);
    try {
      const response = await api.get(`/books`, { params: { page, page_size: PAGE_SIZE } });
      const newBooks: IBook[] = response.data.books;

      setBooks((prevBooks) => {
        const existingBookIds = new Set(prevBooks.map(book => book.id));
        const filteredNewBooks = newBooks.filter(book => !existingBookIds.has(book.id));
      ...
      if (axiosError.response) {
        message.error('Failed to load books: ' + (axiosError.response.data.message || 'Network error'));
      } else {
  ...
  const handleCreateBook = async (title: string) => {
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        message.error('Failed to add book: ' + error.response.data.message || 'Network error');
      } else {
  ...
  const updateBookTitle = async (id: number, newTitle: string) => {
    try {
      ...
      setBooks((prevBooks) => {
        return prevBooks.map((book) => (book.id === id ? { ...book, title: newTitle } : book));
      });
      message.success('Book title updated successfully.');
    } catch (error) {
      const axiosError = error as IAxiosError;
      message.error('Failed to update book title: ' + (axiosError.response?.data.message || 'Network error'));
    }
  };
  ...
  const updateBookRating = async (id: number, rating: number) => {
    try {
      ...
      setBooks((prevBooks) => {
        return prevBooks.map((book) => (book.id === id ? { ...book, rating } : book));
      });
      message.success('Book rating updated successfully.');
    } catch (error) {
      const axiosError = error as IAxiosError;
      message.error('Failed to update book rating: ' + (axiosError.response?.data.message || 'Network error'));
    }
  };
  ...
  const handleDeleteBook = async (id: number) => {
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        message.error('Failed to delete book: ' + error.response.data.message || 'Network error');
      } else {
  ...
  return (
    <div style={{ padding: '20px', fontSize: '18px' }}>
      ...
      >
        ...
        onDelete={handleDeleteBook} onRatingChange={updateBookRating} onTitleChange={updateBookTitle} />
      </div>
    </div>
  );
};
